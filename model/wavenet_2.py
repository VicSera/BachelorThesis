...
class WaveNetModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 end_channels=512,
                 classes=256,
                 output_length=Config.out_seq_len,
                 kernel_size=2,
        ...
        self.filter_cond_convs = nn.ModuleList()

        # Right
        self.gate_convs = nn.ModuleList()
        self.gate_cond_convs = nn.ModuleList()
        ...
        self.start_conv = nn.Conv1d(in_channels=self.classes,
                                    ...
                                    kernel_size=1,
                                    bias=bias)
        self.cond_conv = nn.Conv1d(in_channels=self.classes,
                                   out_channels=residual_channels,
                                   kernel_size=1,
                                   bias=bias)
        ...
        for b in range(blocks):
            ...
            for i in range(layers):
                ...
                self.filter_convs.append(nn.Conv1d(in_channels=residual_channels,
                                                   ...
                                                   bias=bias))
                self.filter_cond_convs.append(nn.Conv1d(in_channels=residual_channels,
                                                        out_channels=dilation_channels,
                                                        kernel_size=kernel_size,
                                                        bias=bias))
                ...
                self.gate_convs.append(nn.Conv1d(in_channels=residual_channels,
                                                 ...
                                                 kernel_size=kernel_size,
                                                 bias=bias))
                self.gate_cond_convs.append(nn.Conv1d(in_channels=residual_channels,
                                                      out_channels=dilation_channels,
                                                      kernel_size=kernel_size,
                                                      bias=bias))
    ...
    def wavenet(self, input, cond, dilation_func):
        ...
        for i in range(self.blocks * self.layers):
            ...
            c = dilation_func(c, dilation, init_dilation, i)

            # Left
            filter = self.filter_convs[i](residual)
            filter_cond = self.filter_cond_convs[i](c)
            filter_activated = torch.tanh(filter + filter_cond)

            # Right
            gate = self.gate_convs[i](residual)
            gate_cond = self.gate_cond_convs[i](c)
            gate_activated = torch.sigmoid(gate + gate_cond)

            x = filter_activated * gate_activated
    ...
    def forward(self, input, cond):
        assert cond.size(-1) == input.size(-1)

        x = self.wavenet(input, cond, dilation_func=self.wavenet_dilate)
    ...
    def generate(self, num_samples, cond, first_samples=None, temperature=1.):
        self.eval()
        assert cond.size(-1) == num_samples
        ...
        for i in range(num_samples):
            ...
            x = self.wavenet(input, cond, dilation_func=self.wavenet_dilate)[:, :, -1].squeeze()
