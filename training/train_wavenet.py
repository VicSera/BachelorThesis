...
from training.util import is_mulaw_quantize, is_mulaw
...
from model.util import discretized_mix_logistic_loss, sample_from_discretized_mix_logistic, mix_gaussian_loss, sample_from_mix_gaussian

import training.audio as audio
...
class _NPYDataSource(FileDataSource):
    def __init__(self, dump_root, col, typ="", max_steps=8000,
...
def eval_model(global_step, device, model, y, c, input_lengths, eval_dir, ema=None):
    if ema is not None:
    ...
    with torch.no_grad():
        y_hat = model.incremental_forward(
            initial_input, c=c, T=length, softmax=True, quantize=True, tqdm=tqdm,
...
def __train_step(device, phase, epoch, global_step, global_test_step,
    ...
    if use_cuda:
        ...
        y_hat, _outputs, _replicas = data_parallel_workaround(model, (x, c, False))
    else:
        y_hat = model(x, c, False)
    ...
    if train and step > 0 and step % hparams.checkpoint_interval == 0:
        save_states(step, y_hat, y, input_lengths, checkpoint_dir)
        save_checkpoint(device, model, optimizer, step, checkpoint_dir, epoch, ema)

    if do_eval:
        # NOTE: use train step (i.e., global_step) for filename
        eval_model(global_step, device, model, y, c, input_lengths, eval_dir, ema)
...
def train_loop(device, model, data_loaders, optimizer, checkpoint_dir=None):
    if is_mulaw_quantize(hparams.input_type):
    ...
    while global_epoch < hparams.nepochs:
        for phase, data_loader in data_loaders.items():
            ...
            for step, (x, y, c, input_lengths) in tqdm(enumerate(data_loader)):
                ...
                running_loss += __train_step(device,
                                             ...
                                             optimizer, criterion, x, y, c, input_lengths,
...
def build_model():
    ...
    model = WaveNet(
        ...
        skip_channels=hparams.skip_out_channels,
        local_conditioning_channels=hparams.cin_channels,
        dropout_probability=hparams.dropout,
...
def get_data_loaders(dump_root, test_shuffle=True):
    ...
    for phase in ["train_no_dev", "dev"]:
        ...
        if local_conditioning:
            condition_track = FileSourceDataset(
                RawAudioDataSource(join(dump_root, phase),
                                  max_steps=max_steps, cin_pad=hparams.cin_pad,
                                  hop_size=audio.get_hop_size()))
            assert len(X) == len(condition_track)
            print("Local conditioning enabled. Shape of a sample: {}.".format(
                condition_track[0].shape))
        else:
        ...
        dataset = PyTorchDataset(X, condition_track)
...
if __name__ == "__main__":
    ...
    ema = None
    try:
        ema = train_loop(device, model, data_loaders, optimizer,
